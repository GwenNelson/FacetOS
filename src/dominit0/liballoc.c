...
static inline int getexp( unsigned int size )
{
	...
	{
		#ifdef DEBUG
		klog(LOG_DEBUG,"getexp returns -1 for %i less than MINEXP\n", size );
		#endif
	...
	#ifdef DEBUG
	klog(LOG_DEBUG,"getexp returns %i (%i bytes) for %i size\n", shift - 1, (1<<(shift -1)), size );
	#endif
...
static void dump_array()
{
	...
	klog_lock();
	klog(LOG_DEBUG,"------ Free pages array ---------\n");
	klog(LOG_DEBUG,"System memory allocated: %i\n", l_allocated );
	klog(LOG_DEBUG,"Memory in used (malloc'ed): %i\n", l_inuse );

		for ( i = 0; i < MAXEXP; i++ )
		{
			klog(LOG_DEBUG,"%.2i(%i): ",i, l_completePages[i] );
	
			tag = l_freePages[ i ];
			while ( tag != NULL )
			{
				if ( tag->split_left  != NULL  ) klog(LOG_DEBUG,"*");
				klog(LOG_DEBUG,"%i", tag->real_size );
				if ( tag->split_right != NULL  ) klog(LOG_DEBUG,"*");
	...
	klog(LOG_DEBUG,"'*' denotes a split to the left/right of a tag\n");
	klog_unlock();
}
#endif
...
static struct boundary_tag* allocate_new_tag( unsigned int size )
{
	...
	struct boundary_tag *tag;
		...
		#ifdef DEBUG
		klog(LOG_DEBUG,"Resource allocated %x of %i pages (%i bytes) for %i size.\n", tag, pages, pages * l_pageSize, size );

		l_allocated += pages * l_pageSize;

		klog(LOG_DEBUG,"Total memory usage = %i KB\n",  (int)((l_allocated / (1024))) );
		#endif
...
void *liballoc_malloc_impl(size_t size)
{
	int index;
	void *ptr;
	struct boundary_tag *tag = NULL;
	...
	liballoc_lock();
		...
		if ( ((int)(remainder) > 0) /*&& ( (tag->real_size - remainder) >= (1<<MINEXP))*/ )
		{
			...
			{
				#ifdef DEBUG
				klog(LOG_DEBUG,"Seems to be splittable: %i >= 2^%i .. %i\n", remainder, childIndex, (1<<childIndex) );
				#endif
				...
				#ifdef DEBUG
				klog(LOG_DEBUG,"Old tag has become %i bytes, new tag is now %i bytes (%i exp)\n", tag->real_size, new_tag->real_size, new_tag->index );
				#endif
			}	
		}
...
void liballoc_free_impl(void *ptr)
{
	int index;
	struct boundary_tag *tag;
	...
	liballoc_lock();
		...
		{
			#ifdef DEBUG
			klog(LOG_DEBUG,"Melting tag right into available memory. This was was %i, becomes %i (%i)\n", tag->real_size, tag->split_right->real_size + tag->real_size, tag->split_right->real_size );
			#endif
		...
		{	
			...
			if ( l_completePages[ index ] == MAXCOMPLETE )
			{
				...
				#ifdef DEBUG
				l_allocated -= pages * l_pageSize;
				klog(LOG_DEBUG,"Resource freeing %x of %i pages\n", tag, pages );
				dump_array();
				#endif
	...
	#ifdef DEBUG
	klog(LOG_DEBUG,"Returning tag with %i bytes (requested %i bytes), which has exponent: %i\n", tag->real_size, tag->size, index ); 
	dump_array();
	#endif
...
void* liballoc_calloc_impl(size_t nobj, size_t size)
{
       int real_size;
       void *p;
       ...
       p = liballoc_malloc_impl( real_size );
...
void*   liballoc_realloc_impl(void *p, size_t size)
{
	void *ptr;
	struct boundary_tag *tag;
	int real_size;
	...
	if ( size == 0 )
	{
		liballoc_free_impl( p );
		return NULL;
	}
	if ( p == NULL ) return liballoc_malloc_impl( size );
