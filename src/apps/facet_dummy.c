...
int main(int argc, char **argv)
{
    ...
    FacetString lifecycle_name = {.data = "process.lifecycle", .length = 17};
    ...
    if (environment == NULL ||
        ...
        environment->resolve(environment->self, &lifecycle_name,
    ...
    IProcessLifecycle *lifecycle = libfacet_proxy_from_handle(
        &IProcessLifecycle_MetaData, lifecycle_handle);
    if (allocator == NULL || facet_app_allocator_use_pages(allocator) != 0)
        return 1;
    ...
    if (output == NULL ||
        ...
        return 1;
    if (lifecycle == NULL ||
        lifecycle->notify_exit(lifecycle->self, 0) != FACET_OK)
        return 1;
    return 0;
}
